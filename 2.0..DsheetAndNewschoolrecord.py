...
tableDropQuery = "DROP TABLE IF EXISTS bugs.sameMobieNoCountBetweenDssheetNewschoolrecord"
cursor.execute(tableDropQuery)
db.commit()

tableCreateQuery = """
    CREATE TABLE bugs.sameMobieNoCountBetweenDssheetNewschoolrecord (
    ds_school_code VARCHAR(13) NOT NULL,
    nsr_school_code VARCHAR(13) NOT NULL,
    DisbursementSheetRowCount INT (5),
    NewschoolrecordDataRowCount INT (5),
...
for row in result:
    ...
    query = """
        ...
        FROM
            bugs.dssheetSchoolCodeMobNo ds1
                INNER JOIN
            bugs.newschoolrecordSchoolCodeMobNo ds2 ON ds1.mobile_no = ds2.MobileNo
        WHERE
    ...
    for rownow in resultNow:
        try:
            dsCntMap[rownow[0]] + nsrCntMap[rownow[1]]
        except KeyError:
            print "One of the School codes not in dssheet or receiveddata"
            continue

        queryNow = """
             INSERT INTO sameMobieNoCountBetweenDssheetNewschoolrecord (
             ds_school_code,
            nsr_school_code,
            ...
            same_mobile_no_count)
              VALUES
              (\"""" + rownow[0] + """\",\""""+ rownow[1] +"""\",""" + str(dsCntMap[rownow[0]]) + """,""" + str(nsrCntMap[rownow[1]]) +\
                   ""","""+ str(rownow[2]) +""")
         """
