...
for schoolCode in schoolCodeList:
    ...
    sys.stdout.write('\r' + "For school no: " + str(count) + " of " + str(len(schoolCodeList)) + " schools, schoolCode: " + schoolCode + " .")
    sys.stdout.flush()
    if schoolCode.strip() == '':
        continue
    query = """
    ...
    db.commit()
    sys.stdout.write("\r" + "Done for schoolCode " + str(schoolCode))
    sys.stdout.flush()
cursor.close()
db.close()